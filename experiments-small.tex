...
In Figure \ref{fig:compare-greedy}, we show the influence---i.e., the objective score in IM---of \textsc{Greedy} and \algomaxcrit{} for various values of $k$. We observe that \algomaxcrit{} has better performance than \textsc{Greedy} in ``Minnesota'' and ``US Power Grid'', and the gap between the two algorithm widens as $k$ increases. Interestingly, there is no noticeable difference in performance in ``Arxiv GrQC'', and our algorithm finds solutions of only slightly higher quality in Facebook. We find this result to be reasonable, since networks with low-dimensional embedding are more likely to benefit from our methods.
